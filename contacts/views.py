...
def inquiry(request):
    if request.method == 'POST':
        ...
        ruser = User.objects.get(id=car.user.id)
        ...
        state=state, email=email, phone=phone, message=message , ruser_id = ruser)

        admin_info = User.objects.filter(is_superuser=True)[0]
